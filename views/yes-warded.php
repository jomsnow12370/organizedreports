...
$families = get_array("SELECT
    ...
    (
        ...
        FROM v_info vi
        INNER JOIN barangays b ON b.id = vi.barangayId
        INNER JOIN head_household hh ON hh.fh_v_id = vi.v_id
        WHERE vi.record_type = 1
        AND vi.v_mname = t1.v_lname
        $munquery $brgyquery
    ) AS total
...
$total_warded = get_value("SELECT COUNT(*) + (SELECT COUNT(*) 
    FROM v_info 
    INNER JOIN household_warding ON household_warding.mem_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 $munquery $brgyquery) as total
    FROM v_info 
    INNER JOIN head_household ON head_household.fh_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 $munquery $brgyquery")[0];

$male_warded = get_value("SELECT COUNT(*) + (SELECT COUNT(*) 
    FROM v_info 
    INNER JOIN household_warding ON household_warding.mem_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 AND v_gender = 'M'  $munquery $brgyquery) as total
    FROM v_info 
    INNER JOIN head_household ON head_household.fh_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 AND v_gender = 'M'  $munquery $brgyquery")[0];

$female_warded = get_value("SELECT COUNT(*) + (SELECT COUNT(*) 
    FROM v_info 
    INNER JOIN household_warding ON household_warding.mem_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 AND v_gender = 'F'  $munquery $brgyquery) as total
    FROM v_info 
    INNER JOIN head_household ON head_household.fh_v_id = v_info.v_id 
    INNER JOIN barangays ON barangays.id = v_info.barangayId 
    WHERE record_type = 1 AND v_gender = 'F'  $munquery $brgyquery")[0];
                    ...
                    <table class="table table-striped">
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <th colspan="3">
                                    <div class="alert bg-info">Voters without birthday data are added to 18-24</div>
                                </th>
                            </tr>
                        </tfoot>
                    </table>
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="barangaySummaryTableforWarded" class="table table-bordered mb-0">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Barangay</th>
                ...
                <tbody>
                    ...
                    foreach ($barangays as $barangay) {
                        ...
                        $warded_brgy = get_value("SELECT COUNT(*) + (SELECT COUNT(*) 
                                            FROM v_info 
                                            INNER JOIN household_warding ON household_warding.mem_v_id = v_info.v_id 
                                            INNER JOIN barangays ON barangays.id = v_info.barangayId 
                                            WHERE record_type = 1 AND barangayId = $brgyId) as total
                                            FROM v_info 
                                            INNER JOIN head_household ON head_household.fh_v_id = v_info.v_id 
                                            INNER JOIN barangays ON barangays.id = v_info.barangayId 
                                            WHERE record_type = 1 AND barangayId = $brgyId")[0];
