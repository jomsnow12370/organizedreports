...
<?php
$status_class = $percent_not_warded >= 80 ? 'success' :
               ($percent_not_warded >= 50 ? 'primary' :
               ($percent_not_warded >= 30 ? 'info' :
               ($percent_not_warded > 0 ? 'warning' : 'danger')));

$status_icon = $percent_not_warded >= 80 ? 'check-circle' :
...
$status_message = $percent_not_warded >= 80 ? 'Excellent! Most voters have been assigned to households.' :
                ($percent_not_warded >= 50 ? 'Good progress. More than half of voters are assigned.' :
                ($percent_not_warded >= 30 ? 'Progress is being made, but many voters are still unassigned.' :
                ($percent_not_warded > 0 ? 'Significant work needed. Only a small percentage of voters are assigned.' :
...
<div class="row mb-4">
    <div class="col-lg-8 col-md-12">
        <div class="card shadow border-0 mb-4">
            <div class="card-body">
                <h5 class="fw-bold mb-3">Top Families Distribution</h5>
                <canvas id="familiesDistributionChart" height="300"></canvas>
            </div>
        </div>
    </div>
    <div class="col-lg-4 col-md-12">
        <div class="card shadow border-0 mb-4">
            <div class="card-body">
                <h5 class="fw-bold mb-3">Top Families</h5>
                <div class="table-responsive">
                    <table class="table table-striped">
                        <thead class="table-primary-outline">
                            <tr>
                        ...
                        <tbody>
                            <?php 
                            $top_families_count = 0;
                            $other_families_count = 0;
                            $family_display_limit = 5; // Show top 5 families
                            
                            foreach ($families as $index => $family): 
                                if ($index < $family_display_limit):
                                    $top_families_count += $family['total'];
                            ?>
                            <tr>
                                <td><?php echo $family['v_lname']; ?></td>
                                <td><?php echo $family['total']; ?></td>
                            </tr>
                            <?php 
                                else:
                                    $other_families_count += $family['total'];
                                endif;
                            endforeach; 
                            ?>
                            <?php if (count($families) > $family_display_limit): ?>
                            <tr>
                                <td>Others</td>
                                <td><?php echo $other_families_count; ?></td>
                            </tr>
                            <?php endif; ?>
                        </tbody>
                        <tfoot class="table-light">
                            <tr>
                                <th>Total</th>
                                <th><?php echo $top_families_count + $other_families_count; ?></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
...
<div class="row mb-4">
    ...
    for ($i = 0; $i < min(4, count($families)); $i++):
        $family = $families[$i];
        ...
        <div class="table-responsive">
            <table id="barangaySummaryTable" class="table table-bordered mb-0">
                ...
                <tbody>
                    <?php
                    $i = 1;
                    $barangays = get_array("SELECT barangay, id FROM barangays WHERE id IS NOT NULL $munquery $brgyquery LIMIT 1");
